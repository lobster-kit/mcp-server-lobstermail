...
2. `send_email` — requires Tier 1+ (Free Verified or above). You can send HTML-only emails (`body_html` without `body_text`); a plain text fallback is auto-generated by stripping tags.
...
| `check_inbox` | Quickly scan for recent emails. Supports `from` filter. |
