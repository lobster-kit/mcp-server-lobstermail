...
You can send HTML-only emails — `body_text` is optional. When only `body_html` is provided, a plain text fallback is auto-generated by stripping tags. At least one of `body_text` or `body_html` must be provided.

Sending requires Free Verified tier or above. To verify:
1. **X verification:** `POST /v1/verify/x` with your tweet URL (free, instant). Requires `X_API_KEY` to be configured in production.
...
| `check_inbox` | List recent emails — sender, subject, preview. Supports `from` filter. |
